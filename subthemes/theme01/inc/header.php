        ...
        <div class="has-sub">
          ...
          <div class="sub">
            <?php foreach ($item['sub'] as $s): ?>
              ...
              <a href="<?= function_exists('sbt_url') ? sbt_url($s['url']) : $s['url'] ?>"><span class="t"><?= $s['label'] ?></span><span class="d"><?= $s['desc'] ?></span></a>
            <?php endforeach; ?>
          </div>
        </div>
      <?php else: ?>
        <a href="<?= function_exists('sbt_url') ? sbt_url($item['url']) : $item['url'] ?>"<?= $PAGE === $item['key'] ? ' class="current"' : '' ?>><?= $item['label'] ?></a>
      <?php endif; ?>
    <?php endforeach; ?>
  </nav>
...
<div class="drawer" id="drawer">
  ...
  <?php foreach ($NAV as $item): ?>
    <a href="<?= function_exists('sbt_url') ? sbt_url($item['url']) : $item['url'] ?>"><?= $item['label'] ?></a>
    <?php if (!empty($item['sub'])): ?>
      <div class="sub-m">
        <?php foreach ($item['sub'] as $s): ?><a href="<?= function_exists('sbt_url') ? sbt_url($s['url']) : $s['url'] ?>"><?= $s['label'] ?></a><?php endforeach; ?>
      </div>
    <?php endif; ?>
  <?php endforeach; ?>
  <a class="wa" href="<?= $SITE['wa'] ?>"><?= $WA_SVG ?><?= $SITE['whatsapp_label'] ?? 'WhatsApp' ?></a>
