...
function sbt_compat_asset_path( $asset_path, $local_path, $subtheme_key = '' ) {
	...
	$compat_marker = 'SyncBooking compatibility CSS v2: original plus unprefixed selectors.';
	if ( file_exists( $compat_path ) && filemtime( $compat_path ) >= filemtime( $local_path ) ) {
		$compat_head = file_get_contents( $compat_path, false, null, 0, 300 );
		if ( false !== $compat_head && false !== strpos( $compat_head, $compat_marker ) ) {
	...
	$compat_css = "/* {$compat_marker} Generated by SyncBooking Theme " . SBT_VERSION . ". */\n" . $css . "\n\n/* Unprefixed compatibility selectors generated from imported assets/site.css. */\n" . str_replace( '.sbtw-', '.', $css );
...
function sbt_render_header_menu_tab( $data, $overrides, $edit_language = 'en' ) {
	...
	<div class="sbt-panel">
		...
		<h2 style="margin-top:28px;">Theme fonts and palette</h2>
		<p class="sbt-muted">These values control the base colors and fonts used by the active subtheme.</p>
		<div class="sbt-field-grid">
			<?php
			sbt_render_header_field( 'SITE.font_pairing', 'Theme font', $data['SITE']['font_pairing'] ?? 'classic', $overrides, 'select', array(
				'classic' => 'Classic resort - Cormorant + Jost',
				'elegant' => 'Elegant - Georgia + Jost',
				'modern' => 'Modern - Jost full',
		...
		<div class="sbt-palette-grid" style="margin-top:14px;">
			<?php
			sbt_render_header_field( 'SITE.color_bg', 'Page background', $data['SITE']['color_bg'] ?? '#f6f2ea', $overrides, 'color' );
			sbt_render_header_field( 'SITE.color_surface', 'Section background', $data['SITE']['color_surface'] ?? '#fbf8f2', $overrides, 'color' );
			sbt_render_header_field( 'SITE.color_ink', 'Primary text', $data['SITE']['color_ink'] ?? '#2b2723', $overrides, 'color' );
			sbt_render_header_field( 'SITE.color_muted', 'Secondary text', $data['SITE']['color_muted'] ?? '#7d7468', $overrides, 'color' );
			sbt_render_header_field( 'SITE.color_line', 'Lines and borders', $data['SITE']['color_line'] ?? '#e6dfd2', $overrides, 'color' );
			sbt_render_header_field( 'SITE.color_primary', 'Primary button color', $data['SITE']['color_primary'] ?? '#8a463f', $overrides, 'color' );
			sbt_render_header_field( 'SITE.color_primary_deep', 'Dark primary', $data['SITE']['color_primary_deep'] ?? '#6f3631', $overrides, 'color' );
			sbt_render_header_field( 'SITE.color_primary_soft', 'Soft primary', $data['SITE']['color_primary_soft'] ?? '#a8645a', $overrides, 'color' );
			sbt_render_header_field( 'SITE.color_accent', 'Accent / overline', $data['SITE']['color_accent'] ?? '#b47e6e', $overrides, 'color' );
			sbt_render_header_field( 'SITE.color_gold', 'Detail gold', $data['SITE']['color_gold'] ?? '#a98c5b', $overrides, 'color' );
